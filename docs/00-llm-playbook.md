...
| launching the stream's sub-model | `gantry stream launch <prefix> --repo <repo> [--cmd '…'] [--dry-run]` | `cd <worktree> && claude -p "$(cat .gantry/packet.md)" --permission-mode acceptEdits …` (any harness via `--cmd`); `.gantry/packet.md` = the method in one screen + boundaries + parent workorder + the stream's issues + the constraints they resolve to + the map's head + commands; `gantry stream packet <prefix>` regenerates it. A `--brief` file with `## issue: <title>` blocks (optional `refs:`/`type:`/`deps:` lines) pre-files `#<prefix>-0002…` |
...
| **an already-adopted repo on older tools** (its `tools/` predate prefixed stream ids) | `python3 scripts/gantry adopt <repo> --fresh-tools` | overwrites `tools/*.py` + `tools/README.md` with the current copies, re-wires both hook shims (worktree-safe, INDEX refresh), auto-wires an existing `.gantry/deps.json`, re-mints `GRAPH.md`. It never commits (no seed issue to reference) — it prints the `git commit -m "… (#NNNN)"` for you to run against one of that repo's own issues |
...
3. gantry stream launch a42 --repo .        ← cd + claude -p with .gantry/packet.md as the prompt
   (the packet: method in brief, boundaries, #0042 verbatim, #a42-0001… verbatim, the constraints
   they resolve to, GRAPH.md head, commands). Its cwd's CLAUDE.md is auto-loaded too.
