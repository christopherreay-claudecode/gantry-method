...
[ "$(cat "$S/gamma/CLAUDE.md")" = "$(printf '# my agent notes\nkeep it simple')" ] || fail "gamma: adopt wrote into the project's CLAUDE.md"
grep -qi 'gantry' "$S/gamma/CLAUDE.md" && fail "gamma: gantry text in CLAUDE.md"
expect "$S/gamma/GRAPH.md" "# session ritual: tools/README.md"
python3 "$G" check "$S/gamma" 2>&1 | grep -q 'no root-level document points at the ritual' || fail "gamma: check did not notice the missing redirection sentence"
printf '\nSession start: read tools/README.md (the gantry ritual), then GRAPH.md.\n' >> "$S/gamma/CLAUDE.md"
python3 "$G" check "$S/gamma" 2>&1 | grep -q 'no root-level document points' && fail "gamma: check still complains after the sentence was added"
git -C "$S/gamma" add -A && git -C "$S/gamma" commit -q -m "the owner's one sentence (#0001)"
...
[ "$(cat "$WT/CLAUDE.md" 2>/dev/null)" = "$(cat "$S/alpha/CLAUDE.md" 2>/dev/null)" ] || fail "stream: CLAUDE.md differs from the parent's (streams never write it)"
expect "$WT/.gantry/packet.md" "stream a1"
expect "$WT/.gantry/packet.md" "Your boundaries"
